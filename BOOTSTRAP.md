...
Treat the project_local section as a convention, not as a list of projects. It is resolved relative to whichever git repository the agent is editing or committing. Do not create .claude/ or .codex/ directories in every project. Leave the defaults if I use .claude/ and .codex/ for project-local agent files, change them if I use different local paths, or set the arrays to [] if I do not want project-local enforcement.
